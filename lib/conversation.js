...
export class RealtimeConversation {
  ...
  EventProcessors = {
    ...
    'response.audio.delta': (event) => {
      ...
      if (!item) {
        // Queue the audio delta for when the item is created
        if (!this.queuedAudioDeltas[item_id]) {
          this.queuedAudioDeltas[item_id] = [];
        }
        this.queuedAudioDeltas[item_id].push(arrayBuffer);
        return { item: null, delta: { audio: appendValues } };
      }
