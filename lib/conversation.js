  ...
  EventProcessors = {
    ...
    'response.audio_transcript.delta': (event) => {
      ...
      if (!item) {
        ...
        // );
        console.warn(`response.audio_transcript.delta: Item "${item_id}" not found, returning empty data`);
        return {
          item: {
            id: item_id,
            content: [{ transcript: '' }],
    ...
    'response.audio.delta': (event) => {
      ...
      if (!item) {
        ...
        console.warn(`response.audio.delta: Item "${item_id}" not found, returning empty data`);
       // Return an empty response instead of throwing an error
        return {
          item: {
            id: item_id,
            content: [{ audio: '' }],
            formatted: { audio: new Int16Array() },
          },
          delta: { audio: new Int16Array() },
        };
      }
